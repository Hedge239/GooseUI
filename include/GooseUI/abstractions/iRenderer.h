...
namespace GooseUI
{
    ...
    namespace absractions
    {
        class iRenderer
        {
            ...
            virtual unsigned int uploadTexture(const unsigned char* pixelData, int width, int height, bool crispPixels, bool grayscale) = 0;
            
            virtual void drawRect(float X, float Y, float W, float H, const color &C) = 0;
            virtual void drawTexture(unsigned int ID, float X, float Y, float W, float H, float u0, float u1, float v0, float v1, const color& color) = 0;
